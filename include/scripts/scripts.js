...
function open_iframe(src,settings) {
    ...
    if (!top.$('outerdiv'+num)) {
        ...
        innerHTML='<table id="pop_title'+num+'" cellspacing="0" cellpadding="0" width="100%" style="background:url(\'/images/popup_bg.gif\');">';
        innerHTML+='<tr>';
        innerHTML+='<td height="26" colspan="2"><span id="page_title'+num+'" style="position:absolute;top:6px;left:6px;font-weight:bold;color:#FFFFFF;-moz-user-select:none;'+(num==1?'cursor:move;':'')+'"></span><img src="/images/close.png" alt="Close Window" style="position:absolute;top:6px;right:6px;cursor:pointer;" onclick="close_iframe('+num+');" onmouseover="this.src=\'/images/close_over.png\';" onmouseout="this.src=\'/images/close.png\';" /></td>';
        innerHTML+='</tr>';
        innerHTML+='</table>';
        innerHTML+='<iframe id="innerframe'+num+'" name="innerframe'+num+'" style="height:'+(height-26)+'px;width:100%;display:none;overflow-x:hidden;overflow-y:auto;" scrolling="yes" frameborder="0"  src="/cust_login.php"></iframe>';
        innerHTML+='<div id="loading'+num+'" style="height:'+(height-26)+'px;width:100%;"><center><img src="/images/spinner.gif" border="0" style="margin-top:100px;" /> Loading - please wait</center></div>';
        o.innerHTML=innerHTML;
        if (typeof(Draggable)=='function') { drag_window=new Draggable(o,{starteffect:'',endeffect:''}); }
    } else {
        var o=top.$('outerdiv'+num);
        top.$('innerframe'+num).style.height=(height-26)+'px';
        top.$('innerframe'+num).style.display='none';
        top.$('loading'+num).style.height=(height-26)+'px';
...
function position_iframe(val) {
    ...
    if (height>vars[3] || height<max_height) {
        ...
        top.$('innerframe'+val).style.height=(height-26)+'px';
    }
    top.center_item(top.$('outerdiv'+val),width,height);
    top.$('overlay').style.width=top.document.body.scrollWidth;
    top.$('overlay').style.height=top.document.body.scrollHeight;
}
...
function show_time_picker(input_id,format) {
    ...
    var t=$('time');
    var ts=t.style;
    var html='<select id="time_drop" size="15" style="border:1px solid #999999;width:85px;" onfocus="retain_time_picker=1;" onclick="$(\''+input_id+'\').value=this.value;close_time_picker();" onblur="close_time_picker();">';
    for (i=0;i<24;i++) {
        for (j=0;j<46;j+=15) {
...
if ($("submenulinks")) {
    var sfEls=$("submenulinks").getElementsByTagName("LI");
    for (var i=0;i<sfEls.length;i++) {
        sfEls[i].onmouseover=function() { this.className+=" sfhover"; }
        sfEls[i].onmouseout=function() { this.className=this.className.replace(new RegExp(" sfhover\\b"), ""); }
    }
